...
def is_matrix_right(matrix):
    ...
    flag_sign = None
    for number in Matrix(matrix):
        if number - comparsion_elem != 0:
            if flag_sign == None:
                flag_sign = number - comparsion_elem
        if flag_sign != None:
            if flag_sign * (number - comparsion_elem) < 0:
                return False
        comparsion_elem = number
    return True
